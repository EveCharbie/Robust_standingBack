...
from visualisation import visualisation_closed_loop_2phases
...
def prepare_ocp(biorbd_model_path, phase_time, n_shooting, min_bound, max_bound):
    ...
    # Path constraint
    # pose_salto_tendu_CL = [0, 0, 0, 2.2199, -1.3461, 0.0]
    # pose_salto_groupe_CL = [0, 0, 0, 2.3432, -2.0252, 0.0]
    # pose_salto_tendu = [0, 0, 0, 0.79, 0.70, 2.0144, -1.1036, 0.0]
    # pose_salto_groupe = [0, 0, 0, 0.7966, 0.9233, 2.2199, -1.3461, 0.0]
    ...
    x_bounds[0]["q"][:, 0] = pose_takeout_end
    ...
    x_bounds[1]["u"].max[2, 0] = 2 * np.pi
    x_bounds[1]["u"].min[2, 1] = np.pi / 2
    x_bounds[1]["u"].max[2, 1] = 2 * np.pi
    x_bounds[1]["u"].min[2, 2] = 2 * np.pi - 0.5
    ...
    x_bounds[1]["udot"].min[2, :] = -5
    x_bounds[1]["udot"].max[2, :] = 10

    x_bounds[1]["u"].min[:, -1] = np.array(pose_salto_end_CL) - 0.5
    x_bounds[1]["u"].min[:, -1] = np.array(pose_salto_end_CL) + 0.5
    ...
    x_init.add("u", np.array([pose_salto_start_CL, pose_salto_end_CL]).T, interpolation=InterpolationType.LINEAR, phase=1)
