...
def prepare_ocp(biorbd_model_path, phase_time, n_shooting, min_bound, max_bound):
    ...
    # Phase 0 (Propulsion):
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_COM_VELOCITY, quadratic=False, node=Node.END, weight=-1, axes=Axis.Z, phase=0)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, quadratic=False, weight=1000, min_bound=0.1, max_bound=0.4, phase=0)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, quadratic=True, key="tau", derivative=True, weight=0.0001, phase=0)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, quadratic=True, key="tau", weight=0.0001, phase=0)
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, quadratic=True, key="tau", weight=0.1, phase=1)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, quadratic=True, key="tau", derivative=True, weight=0.1, phase=1)
