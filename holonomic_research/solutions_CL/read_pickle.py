...
# Charger les données
data = pickle.load(open("sol_5_DVG.pkl", "rb"))
...
prr = PhaseRerun(t_span=time)
model = BiorbdModel("../../../Model/Model2D_7Dof_2C_5M_CL_V3.bioMod")
prr.add_animated_model(model, q)
prr.rerun()
...
def create_subplots(data_var, min_bounds, max_bounds, var_name, vertical_idx_offset=0):
    ...
    for row, (i, j) in enumerate(product(range(1, 3), range(1, 5))):
        ...
        fig.add_trace(go.Scatter(x=np.arange(data_var.shape[1]), y=data_var[row, :], mode='markers', name=f'{var_name}{i}'), row=i, col=j)
