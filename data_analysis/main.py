...
insole_calibration_data_R, insole_calibration_data_L = (
    insole_calibration_data_R.iloc[3:, 1:-1],
    insole_calibration_data_L.iloc[3:, 1:-1],
)
insole_calibration_data_R.reset_index(drop=True, inplace=True)
insole_calibration_data_L.reset_index(drop=True, inplace=True)
existing_sensor_index = (
    insole_calibration_data_R.columns.to_list()
)  # List sensor on the insole size 45 (without columns Sync and Time)

insole_coordinates = insoles_coordonnees.loc[0:7, existing_sensor_index]

# Cartography insole
cartography_insole(file_insole=insole_calibration_data_R, file_info_insole=insole_coordinates, FLAG_PLOT=True)

# Find circle
parameters_circle_L = points_to_circle(
    markers_insole_xy=markers_insole_L_xy,
    fig_name="Circle_insole_L",
    markers_name=markers_insole_L_name,
    FLAG_PLOT=True,
)
parameters_circle_R = points_to_circle(
    markers_insole_xy=markers_insole_R_xy,
    fig_name="Circle_insole_R",
    markers_name=markers_insole_R_name,
    FLAG_PLOT=True,
)
...
force_orientation_L = get_force_orientation(
    x_columns_opt=x_columns_opt_L,
    y_columns_opt=y_columns_opt_L,
    center_x=parameters_circle_L[1]["center_x"],
    center_y=parameters_circle_L[1]["center_y"],
    FLAG_PLOT=True,
)
force_orientation_R = get_force_orientation(
    x_columns_opt=x_columns_opt_R,
...
markers_knee_L_mean = np.mean(
    np.array([model.markers(Q)[markers_knee_L_idx[0]].to_array(), model.markers(Q)[markers_knee_L_idx[1]].to_array()]),
    axis=0,
)
markers_knee_R_mean = np.mean(
    np.array([model.markers(Q)[markers_knee_R_idx[0]].to_array(), model.markers(Q)[markers_knee_R_idx[1]].to_array()]),
    axis=0,
)
...
print(
    "The distance between the mean left knee markers and mean insoles markers is : ",
    np.linalg.norm(markers_insole_L_mean - markers_knee_L_mean),
    " m",
)
print(
    "The distance between the mean right knee markers and mean insoles markers is : ",
    np.linalg.norm(markers_insole_R_mean - markers_knee_R_mean),
    " m",
)
...
for filename in os.listdir(data_folder):
    if "salto" in filename and (filename.split("_")[-1] == "L.CSV" or filename.split("_")[-1] == "R.CSV"):
        ...
        if filename[-5] == "L":
            force_data = get_force_from_insoles(
