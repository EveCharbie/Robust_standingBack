...
import numpy as np
from holonomic_research.ocp_example import generate_close_loop_constraint, custom_configure, custom_dynamic
...
def prepare_ocp(biorbd_model_path, phase_time, n_shooting, min_bound, max_bound):
    ...
    bio_model = (BiorbdModel(biorbd_model_path[0]),
            ...
            BiorbdModelCustomHolonomic(biorbd_model_path[4]),
    ...
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=10000, phase=1, min_bound=0.01, max_bound=0.2)
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, key="tau", weight=500, phase=4, derivative=True)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_TIME, weight=10, phase=4, min_bound=0.2, max_bound=1)
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_COM_POSITION, node=Node.ALL_SHOOTING, weight=-100000, phase=4)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_STATE, key="qdot", weight=100, phase=4, derivative=True)
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_STATE, key="q", weight=1, phase=4, index=7)
    ...
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_COM_POSITION, node=Node.END, weight=100, phase=6, axes=Axis.Y)
    ...
    pose_propulsion_start = [-0.2347217373715483, -0.45549996131551357, -0.8645258574574489, 0.4820766547674885, 0.03, 2.590467089448695, -2.289747592408045, 0.5538056491954265]
    # pose_takeout_start = [-0.2777672842694191, 0.03995514292843797, 0.1930477703559439, 2.589642304908377, 0.03, 0.5353536016159908, -0.8367077461678971, 0.11196901833050495]
    pose_takeout_start = [-0.0509, 0.2655, 0.6228, 2.6735, 0.7023, -0.1848, -0.6097, -0.324]
    # pose_takeout_end = [-0.2803, 0.4015, 0.5049, 3.0558, 1.7953, 0.2255, -0.3913, -0.575]
    pose_takeout_end = [-0.2803, 0.4015, 0.6228, 3.0558, 1.7953, 0.2255, -0.3913, -0.575]
    pose_grouper = [-0.3163014992926154, 0.6241792146792007, 0.8221293983724339, 0.8894508807174182, 1.500676749287633, 1.928876307825571, -1.9208153418590037, 0.4438265289723043]
    pose_salto_start = [-0.3269534844623969, 0.681422172573302, 0.9003344030624946, 0.35, 1.43, 2.3561945135532367, -2.300000008273391, 0.6999999941919349]
    pose_salto_end = [-0.8648803377623905, 1.3925287774995057, 3.785530485157555, 0.35, 1.14, 2.3561945105754827, -2.300000018314619, 0.6999999322366998]
    pose_landing_start = [-0.9554004763233065, 0.15886445602166693, 5.832254254152056, -0.45610833795726297, 0.03, 0.6704528346396729, -0.5304889643328282, 0.654641794221728]
    pose_landing_end = [-0.9461201943294933, 0.14, 6.28, 3.1, 0.03, 0.0, 0.0, 0.0]
    # Position of segment during landing
    ...
    x_init.add(np.array([pose_at_first_node + [0] * n_qdot, pose_propulsion_start + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array([pose_propulsion_start + [0] * n_qdot, pose_takeout_start + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array(pose_at_first_node + [0] * n_qdot))
    # x_init.add(np.array([pose_takeout_end + [0] * n_qdot, pose_salto_start + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array([pose_takeout_end + [0] * n_qdot, pose_grouper + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array([pose_salto_start + [0] * n_qdot, pose_salto_end + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array([pose_salto_end + [0] * n_qdot, pose_landing_start + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)
    x_init.add(np.array([pose_salto_start + [0] * n_qdot, pose_landing_end + [0] * n_qdot]).T, interpolation=InterpolationType.LINEAR)

    # Define control path constraint
    u_bounds = BoundsList()
    u_bounds.add([tau_min[3], tau_min[4], tau_min[5], tau_min[6], tau_min[7]], [tau_max[3], tau_max[4], tau_max[5], tau_max[6], tau_max[7]])
    u_bounds.add([tau_min[3], tau_min[4], tau_min[5], tau_min[6], tau_min[7]], [tau_max[3], tau_max[4], tau_max[5], tau_max[6], tau_max[7]])
    u_bounds.add([tau_min[3], tau_min[4], tau_min[5], tau_min[6], tau_min[7]], [tau_max[3], tau_max[4], tau_max[5], tau_max[6], tau_max[7]])
    u_bounds.add([tau_min[3], tau_min[4], tau_min[5], tau_min[6], tau_min[7]], [tau_max[3], tau_max[4], tau_max[5], tau_max[6], tau_max[7]])
    u_bounds.add([tau_min[3], tau_min[4], tau_min[5], tau_min[6], tau_min[7]], [tau_max[3], tau_max[4], tau_max[5], tau_max[6], tau_max[7]])
...
def main():
    ocp = prepare_ocp(
        biorbd_model_path=(str(name_folder_model) + "/" + "Model2D_8Dof_3C_5M.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_2C_6M.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_0C_5M.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_0C_5M.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_0C_5M_CL.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_0C_5M.bioMod",
                           str(name_folder_model) + "/" + "Model2D_8Dof_3C_5M.bioMod",
                           ),
        phase_time=(0.15, 0.05, 0.05, 0.05, 0.4, 0.05, 0.2),
        n_shooting=(15, 5, 5, 5, 40, 5, 20),
        min_bound=50,
        max_bound=np.inf,
    )
