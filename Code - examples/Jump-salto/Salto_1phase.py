...
from holonomic_research.visualisation import visualisation_closed_loop
...
def prepare_ocp(biorbd_model_path, phase_time, n_shooting, min_bound, max_bound):
    ...
    # bio_model.model.set_dependencies(independent_joint_index=[0, 1, 2, 5, 6], dependent_joint_index=[3, 4])
    ...
    # x_bounds[0][:, 0] = pose_salto_tendu + [0] * n_qdot # impose the first position
    # x_bounds[0].min[0, 2] = -1
    # x_bounds[0].max[0, 2] = 1

    # Initial guess
    x_init = InitialGuessList()
    x_init.add(np.array([pose_salto_tendu + [0] * n_independent, pose_salto_groupe + [0] * n_independent]).T, interpolation=InterpolationType.LINEAR)
